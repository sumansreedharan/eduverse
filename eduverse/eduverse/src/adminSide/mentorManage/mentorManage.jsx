...
const MentorList = () =>{
    ...
    const fetchMentors = async()=>{
        try {
            const response = await axios.get("http://localhost:3001/admin/listMentors");
    ...
    const toggleBlock = async (id) => {
      ...
      const config = {
        headers: {
          ...
          await axios.put(`http://localhost:3001/admin/blockMentor/${id}`,null,config);
              ...
              {mentors.map((mentor) => (
                <React.Fragment key={mentor._id}>
                  <tr>
                    ...
                    <td><strong>{mentor.blocked ? "Blocked" : "Active"}</strong></td>
                    <td>
                      <button
